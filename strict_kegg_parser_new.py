#Usage: python3 strict_kegg_parser.py -i path/to/kofamsccan/output -l Nitrogen_list -k kegg_control_file
...
#Read in count file
sample_KO_matrix = pd.read_csv(args.input_dir+'/KEGG_output_count_table.txt', sep="\t", header=0)
sample_KO_matrix_df =sample_KO_matrix.replace(to_replace='_kegg_output_detailed_reliable_tabbed_final', value='',regex=True)
sample_KO_matrix_df = sample_KO_matrix_df.rename(columns={'Genome': 'KO'})
sample_KO_matrix_df = sample_KO_matrix_df.set_index('KO',inplace=False)
sample_KO_matrix_df_transp = sample_KO_matrix_df.transpose()
sample_KO_matrix_df_transp = sample_KO_matrix_df_transp.reset_index(level=0, inplace=False)
sample_KO_matrix_df_transp = sample_KO_matrix_df_transp.rename(columns={'index': 'KO'})
sample_KO_matrix_df_transp.index = sample_KO_matrix_df_transp.index.rename('Index')

#Read in control file
KEGG_control = pd.read_csv(args.input_dir+'/'+args.pathways, sep="\t", header=0)
#Merge dataframes
KEGG_control_df = pd.merge(KEGG_control,sample_KO_matrix_df_transp,on='KO',how='outer')
#Write to file
KEGG_control_df.to_csv(args.input_dir+'/KO_pathway_summary.tab', sep='\t', index=False)

#Delete interim files
int_files = os.listdir(args.input_dir)
for int_file in int_files:
    if int_file.endswith(("_reliable","_tabbed","final")):
